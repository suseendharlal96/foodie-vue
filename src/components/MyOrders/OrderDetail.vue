<template>
  <div>
    ...
    <p :style="{ color: 'var(--primaryTheme)' }">
      Ordered on:
      <strong :style="{ color: 'var(--primaryText)' }">{{new Date(date).toString()}}</strong>
    </p>
    <p :style="{ color: 'var(--primaryTheme)' }">Customer Details:</p>
    <template v-for="(value, propertyName,index) in custDetails">
      <li :key="index" :style="{ color: 'var(--primaryTheme)'}">
        {{propertyName}}
        <span :style="{ color: 'var(--primaryText)' }">:{{value}}</span>
      </li>
    </template>
    <p :style="{ color: 'var(--primaryTheme)' }">Items ordered:</p>
    <template v-for="(ing,index) in menu">
      <li :key="index">
        <strong :style="{ color: 'var(--primaryText)' }">{{ing.name}}</strong>
        <span :style="{ color: 'var(--primaryTheme)' }">
          price:
          <strong :style="{ color: 'var(--primaryText)' }">{{"\u20B9"}} {{ing.price}}(1)</strong>
        </span>
        <span :style="{ color: 'var(--primaryTheme)' }">
          quantity:
          <strong :style="{ color: 'var(--primaryText)' }">{{ing.quantity}}</strong>
        </span>
        <span :style="{ color: 'var(--primaryTheme)' }">
          total:
...
<script>
export default {
  props: ["custDetails", "menu", "total", "date"],
  data() {
    return {
      customerDetails: []
    };
